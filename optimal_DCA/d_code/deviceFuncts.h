...
__device__ void set_Dt_gpu(float D[6][6],int row,int col);
__device__ void set_D_gpu(float D[6][6],int row,int col);
__device__ void transpose_gpu(float S[6][6],int row, int col);
__device__ void makeS_gpu(float S[6][6],float *r);
__device__ void Mat61Mult_gpu(float A[6][6], float B[6][6], float C[6][6],int row, int col);
__device__ void Mat66Mult_gpu(float A[6][6], float B[6][6], float C[6][6],int row, int col);
...
__device__ void get_X_gpu(float z1[6][6], float z2 [6][6], float D[6][6],int row , int col)
{
...
//		row and col are	the threadIdx.y and threadIdx.x
__device__ void invert_X_gpu(float X[6][6], float A[6][6],int row, int col)
{
     int s;
     int j;
     float temp;
...
//		row and col are	the threadIdx.y and threadIdx.x
__device__ void make_W_gpu(float Xinv[6][6], float D[6][6],int row , int col)                    
{
    if (row == 5 || col==5)
    {
...
//		row and col are	the threadIdx.y and threadIdx.x
__device__ void zaa_gpu(float r[],float z[6][6],float Minv[6][6],float S[6][6], int row, int col)
{
...
//		row and col are	the threadIdx.y and threadIdx.x
__device__ void zab_gpu(float r[],float z[6][6],float Minv[6][6],float S[6][6],int row , int col)
{    
    if (row+col==0)	//Ensure only 1 thread enters the if
	{
          makeS_gpu(S,r);	//Create the shifter matrix
...
__device__ void MSsetup_gpu(float Minv[6][6],float S[6][6],int row , int col,float m, float I[],int Iindex)
{
...
//		m is the mass of the body
//		row and col are	the threadIdx.y and threadIdx.x
__device__ void Fa_gpu(float S[6][6],float w, float r[], float m,int row, int col)
{
...
__device__ void r02_gpu(float r[], float angle,float L)
{
    r[0]=L*sin(angle)/2;
    r[1]=-1*L*cos(angle)/2;
    r[2]=0;
...
//		r is the vector in which r02 will be stored
//		angle is the ange of the body with respect to the vertical
__device__ void r01_gpu(float r[], float angle,float L)
{
    r[0]=-1*L*sin(angle)/2;
    r[1]=L*cos(angle)/2;
    r[2]=0;
...
__device__ void printit_gpu(float A[6][6])
{
