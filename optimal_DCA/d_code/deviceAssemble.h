...
__global__ void Assemble_gpu(float oldZs[],float newZs[],float Xinvs[], int nn)
{
	...
	get_X_gpu(z1,z2,A,row,col);//Get the X matrix and save it in A
	invert_X_gpu(A,z1,row,col);//Invert X and save it in A

	if (col!=5 && row !=5)//Xinv is only a 5x5
	{
    	Xinvs[wXinv]=A[row][col];//Save Xinv
	}
	make_W_gpu(A,z1,row,col);//Make W out of X and save it in A
	//A now holds W
	...
	Mat66Mult_gpu(A,z2,z2,row,col);
	Mat66Mult_gpu(z1,z2,z2,row,col);//z2 now holds the new z12
	...
	Mat66Mult_gpu(A,z2,z2,row,col);
	Mat66Mult_gpu(z1,z2,z2,row,col);
	...
	Mat66Mult_gpu(A,z2,z2,row,col);
	Mat66Mult_gpu(z1,z2,z2,row,col);//z2 now holds the new z21
	...
	Mat61Mult_gpu(A,z1,A,row,col);//col = 0 of A now holds Y
