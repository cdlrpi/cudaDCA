...
int main()
{
	...
	float *inits;	//Initial conditions
	...
	Y = new float[2*n];	//Timestep solution is length 2*n
	...
	float Vals[2][n+1];	//Matrix holding information to calculate the energy
	float X[tlen][2*n];	//Solution Matrix
	...
	//Solution Output
	npy_save_float("mat.npy", fortran_order, 2, shape1, &X[0][0]);	//Output the solution
	npy_save_float("Vals.npy",fortran_order,2,shape2,&Vals[0][0]);	//Output values to find energy
