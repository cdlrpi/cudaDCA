...
void DCAhelp(double state[], double m[], double l[], double I[],int n, double Y[],int cut_off, std::vector<Body> body, std::vector<Forces> AF,double Zs[],float times[], int reps);
...
void RK_45(double state[], double step, int n,double m[], double l[], double II[],double Y[],int cut_off,  std::vector<Body> body, std::vector<Forces> AF, double Zs[],float times[], int reps)
{
	...
	DCAhelp(tState,m,l,II,n,Y1,cut_off,body,AF,Zs,times,reps);
/*
	arycpy3(qddot1,Y1,n);
