...
__device__ void set_Dt_gpu(double D[6][6],int row);
__device__ void set_D_gpu(double D[6][6],int row);
__device__ void transpose_gpu(double S[6][6],int row);
...
__device__ void get_X_gpu(double z1[6][6], double z2 [6][6], double D[6][6],int row)
{
    ...
    set_D_gpu(D,row);	//put the D matrix corresponding to a joint in this simulation in D

    Mat66Mult_gpu(z1,D,z1,row);	//Perform z1*D and store the solution in z1
	//z1 now holds (b1.z22+b2.z11)*D

    set_Dt_gpu(D,row);	//put the transpose of the D matrix in D

    Mat66Mult_gpu(D,z1,D,row);//Perform D*z1 and store the solution in D
...
__device__ void set_D_gpu(double D[6][6],int row)
{
    ...
    if(row==0)	//Ensure only one thread enters the if statement
    {
...
__device__ void make_W_gpu(double Xinv[6][6], double D[6][6],int row)                    
{
         ...
         Xinv[row][5]=0;	//Set the values in row and column 6 to 0
		Xinv[5][row]=0;

    set_Dt_gpu(D,row);	//Put the D transpose matrix in D
    Mat66Mult_gpu(Xinv,D,Xinv,row);	//Perform Xinv*D and store the solution in Xinv
	//Xinv now holds Xinv*Dt

    set_D_gpu(D,row);	//Put the D matrix in D
    Mat66Mult_gpu(D,Xinv,Xinv,row);	//Perform D*Xinv and store the solution in Xinv
...
__device__ void Mat66Mult_gpu(double A[6][6], double B[6][6], double C[6][6],int row)
{
    ...
    while (l<6)
    {
        ...
        j+=A[row][l]*B[l][ind];	//dot product of row and column
