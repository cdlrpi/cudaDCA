...
__device__ void get_X(float z1[6][6], float z2 [6][6], float D[6][6],int row , int col)
{
    ...
    set_D(D,row,col);	//put the D matrix corresponding to a joint in this simulation in D

    Mat66Mult(z1,D,z1,row,col);	//Perform z1*D and store the solution in z1
	//z1 now holds (b1.z22+b2.z11)*D

    set_Dt(D,row,col);	//put the transpose of the D matrix in D

    Mat66Mult(D,z1,D,row,col);//Perform D*z1 and store the solution in D
...
__device__ void make_W(float Xinv[6][6], float D[6][6],int row , int col)                    
{
    ...
    set_Dt(D,row,col);	//Put the D transpose matrix in D
    Mat66Mult(Xinv,D,Xinv,row,col);	//Perform Xinv*D and store the solution in Xinv
	//Xinv now holds Xinv*Dt

    set_D(D,row,col);	//Put the D matrix in D
    Mat66Mult(D,Xinv,Xinv,row,col);	//Perform D*Xinv and store the solution in Xinv
...
__device__ void zaa(float r[],float z[6][6],float Minv[6][6],float S[6][6], int row, int col)
{
     ...
     Mat66Mult(Minv,S,z,row,col);	//Perform Minv*S and store the solution in z
	//z now holds Minv*S
     
     transpose(S,row,col);	//Transpose the shifter matrix
     Mat66Mult(S,z,z,row,col);	//Perform S*z and store the solution in z
	//z now holds St*Minv*S=zaa

     Mat66Mult(S,Minv,Minv,row,col);	//Perform S*Minv and store the solution in Minv
